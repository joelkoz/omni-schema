...
OmniSchema.joi.defineJoiType = function (name, joiType, props) {
	var joiSpec = { joiSpec: Object.assign({ type: joiType }, props) };
	OmniTypes.defineDataType(name, joiSpec);
};

var plugin = function plugin() {

	var defineJoiType = OmniSchema.joi.defineJoiType;
	...
	OmniSchema.mixin({
		...
		onSchema: [{
			func: function getJoiSchemaDef(uiExclude) {
				...
				for (var fieldName in omniSchema) {
					...
					if (field instanceof OmniSchema.OmniField && (!uiExclude || !field.uiExclude)) {
						var joiField = field.getJoiField();
			...
			func: function getValidator(uiExclude) {
				var joiSchemaDef = this.getJoiSchemaDef(uiExclude);
				return {
					...
					validate: function validate(value, ignoreFields) {
						...
						var result = this.schema.validate(_.omit(value, ignoreFields));
					...
					isValid: function isValid(value, ignoreFields) {
						try {
							var result = this.validate(value, ignoreFields);
							return result;
						} catch (error) {
		...
		onType: [{ matches: 'joiSpec',
			...
			func: function getJoiField(field, controlProps) {
				...
				if (this.joiSpec.type === 'string' && this.enumValues) {
					var enumVals = this.enumValues.map(function (ev) {
						return ev.value;
					});
					if (!field.isRequired) {
						enumVals.push(null);
